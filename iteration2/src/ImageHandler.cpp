    ...
    if (hasSuffix(fileName, ".png")) {
        ...
        if (png_image_begin_read_from_file(&image, filePath) != 0) {
            ...
            if (pngBuffer != NULL && png_image_finish_read(&image, NULL, pngBuffer, 0, NULL) != 0)
            {
                ...
                PixelBuffer* replace = new PixelBuffer((int) image.width, (int) image.height, buf->getBackgroundColor());
                ...
                /* Replace buf and return successfully */
                buf = replace;
                return 1;
            }
            else { return 0; }    // Error
        ...
        PixelBuffer* replace = new PixelBuffer(dinfo.output_width, dinfo.output_height, buf->getBackgroundColor());
